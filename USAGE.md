...
Report configuration can add a customer name, title, prepared-by value, notes, and an optional logo. These values stay with a saved dataset. Use **Export PDF / Print** for a clean light-theme print layout, **Download HTML** for a self-contained offline report, or the chart controls to download individual SVG and PNG files. **Download Monthly Data CSV** exports the trend rows for further analysis.
...
-   For multi-file trend analysis, use non-overlapping export windows. Matching unique connections are deduplicated across files, but flow totals from overlapping exports cannot be separated and will be added more than once.
